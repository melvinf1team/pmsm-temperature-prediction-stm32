...
/*
 * On garde 750 ms pour rester compatible 12 bits.
 * Ce temps n'est pas bloquant : app_datalog attend avec HAL_GetTick().
 */
...
static uint32_t ds18b20_fresh_ok_count = 0U;
static uint32_t ds18b20_fresh_fail_count = 0U;
static uint32_t ds18b20_cache_used_count = 0U;
...
static void DS18B20_InitModule(void)
{
  ...
  ds18b20_fresh_ok_count = 0U;
  ds18b20_fresh_fail_count = 0U;
  ds18b20_cache_used_count = 0U;
...
static bool DS18B20_ReadTemperatureCentiFresh(int16_t *temperature_centi)
{
  ...
  ds18b20_fresh_fail_count++;

  return false;
}

static void DS18B20_FormatCenti(char *dst, size_t dst_len, int16_t temp_centi)
{
...
static bool DS18B20_ReadCsvValues(char *dst, size_t dst_len)
{
  ...
  /*
   * 1. On tente une vraie lecture fraîche.
   */
  if (DS18B20_ReadTemperatureCentiFresh(&temp_centi))
  {
    DS18B20_FormatCenti(dst, dst_len, temp_centi);
    return true;
  }

  /*
   * 2. Si la lecture fraîche échoue mais qu'on a une ancienne valeur,
   * on renvoie la dernière valeur valide au lieu de nan.
   *
   * Avantage : le CSV reste exploitable.
   * Inconvénient : la valeur peut être légèrement stale.
   */
  if (ds18b20_has_last_valid)
  {
    ds18b20_cache_used_count++;
    DS18B20_FormatCenti(dst, dst_len, ds18b20_last_temp_centi);
    return true;
  }

  /*
   * 3. Seulement si aucune lecture valide n'a jamais été obtenue.
   */
...
const DatalogModule_t DS18B20_Module =
{
  ...
  .conversion_time_ms = DS18B20_CONVERSION_MS
};
