...
static int16_t d6tir_last_raw_tenth = 0;
...
static bool D6TIR_ReadTemperature(void)
{
  ...
  d6tir_last_raw_tenth = raw_tenth;
...
void D6TIR_Init(void)
{
  ...
  first_read_ok = D6TIR_ReadTemperature();
  d6tir_next_read_ms = HAL_GetTick() +
                       (first_read_ok ? D6TIR_PERIOD_PRESENT_MS : D6TIR_PERIOD_MISSING_MS);
}
...
{
  if ((temperature_c == NULL) || !d6tir_has_value)
  {
    return false;
  }

  *temperature_c = (float)d6tir_last_raw_tenth / 10.0f;
  return true;
}